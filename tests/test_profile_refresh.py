...
class ProfileRefreshContractTests(unittest.TestCase):
    def test_basics_refreshes_summary_and_cv_without_signals_dependency(self):
    ...
    def test_projects_refreshes_summary_without_cv_preview_or_signals_dependency(self):
    ...
    def test_credentials_refreshes_summary_and_cv_without_transitional_signals_refresh(self):
        plan = build_profile_refresh_plan(ProfileRefreshAction.CREDENTIALS)

        self.assertEqual(plan.primary_template, "profile/_formation_shell.html")
        self.assertEqual(plan.redirect_section, ProfileSectionId.CREDENTIALS)
        self.assertTrue(plan.refresh_summary)
        self.assertFalse(plan.refresh_signals)
        self.assertTrue(plan.refresh_cv_preview)
